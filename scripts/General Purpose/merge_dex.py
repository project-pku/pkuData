# A python script that merges two json files
import commentjson
import os
...
# merge datadexes
base = commentjson.load(open(os.path.join(root, "dexA.json"), encoding="utf8"))
new = commentjson.load(open(os.path.join(root, "dexB.json"), encoding="utf8"))
merge_dicts(base, new)

# merging complete, write file
with open(os.path.join(root, "mergedDex.json"), 'w', encoding='utf-8') as f:
  f.write(to_json(base))
    
input("\nFinished merging dexes!")